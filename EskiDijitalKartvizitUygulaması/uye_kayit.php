...
OB_START();
include 'baglan.php';
$kartsec = $baglan->prepare("SELECT * FROM  `kartvizit` WHERE `durum` ='0' AND `bayi_id` ='3' ORDER BY `id` DESC limit 0,1");
$kartsec ->execute();
$kartim = $kartsec->fetch(PDO::FETCH_ASSOC);
...
<h5 class="card-title text-center pb-0 fs-4" style="color:#fff;">Üye Kayıt Formu</h5>
<p class="text-center small" style="color:#fff;">DijitaCO'yu Deneyimlemek İçin Lütfen Aşağıda ki Bilgileri Doldurunuz</p>
</div>
...
<div class="col-12">
<button class="btn btn-light w-100" type="submit">KAYIT</button>
</div>
</form>
<?php 
}else{
...
$bitis 		=date('d.m.Y', strtotime("+7 days"));
$sifre =md5($parola);
...
$guncelle =$baglan->prepare("UPDATE `kartvizit` SET `uye_id`=?,`profil`=?,`isim`=?,`soyisim`=?,`telefon`=?,`email`=?,`durum`=?,`tarih`=?,`bitis`=? WHERE `kart_id`=?");
$guncelle ->execute(array($uyeidim,$profil,$isim,$soyisim,$telefon,$email,$durum,$tarih,$bitis,$kart_id));
/* LOG */
...
$mesaj ='Sayın '.$isim.' '.$soyisim.' DijitaCO’yu Tercih Ettiğiniz İçin Teşekkür Ederiz. 7 Günlük Deneme Süreniz Başlamıştır. Giriş Paneliniz : https://vcard.dijitaco.com/giris Kullanıcı Adı : '.$username.' Şifre : '.$parola.'  Hesabınıza Giriş Yaptıktan Sonra Dijital Kartlvizitlerinizi Özelleştirebilirsiniz.';

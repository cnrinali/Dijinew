...
<?php 
if(empty($_POST)){
$kartvizitbul	=$baglan->prepare("SELECT * FROM `sosyal_medya` WHERE `id` =?");
$kartvizitbul 	->execute(array($id));
$klistele		=$kartvizitbul ->FETCH(PDO::FETCH_ASSOC);
$kart_id		=$klistele["kart_id"];
$sosyal			=$klistele["sosyal"];
$hesap			=$klistele["hesap"];
...
?>				
    <form name="form" action="" method="POST">
				   <div class="row mb-3">
                    ...
                    <div class="row mb-3">
                      <label for="fullName" class="col-md-4 col-lg-3 col-form-label">Sosyal Medya</label>
                      <div class="col-md-8 col-lg-9">
                        <input name="sosyal" type="text" class="form-control" id="isim" value="<?php echo"$sosyal";?>" readonly>
                      </div>
                    </div>
                    <div class="row mb-3">
                      <label for="fullName" class="col-md-4 col-lg-3 col-form-label">Hesap</label>
                      <div class="col-md-8 col-lg-9">
                        <input name="hesap" type="text" class="form-control" id="isim" value="<?php echo"$hesap";?>" required>
                      </div>
                    </div>
...
$guncelle =$baglan->prepare("UPDATE `sosyal_medya` SET `hesap`=?,`sosyal`=? WHERE `id`=?");
$guncelle ->execute(array($hesap,$sosyal,$id));
if (isset ($guncelle)){

/* LOG */
$islem ="Sosyal Medya İçeriği Güncellendi ($kart_id)";

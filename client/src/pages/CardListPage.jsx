...
function CardListPage() {
  ...
  const handleToggleStatus = async (cardId, currentStatus) => {
    ...
    try {
      ...
      setCards((prevCards) =>
        prevCards.map((c) =>
          c.id === cardId ? { ...c, isActive: result.card.isActive } : c
        )
      );
      ...
      const errorMsg =
        err.response?.data?.message || "Kart durumu değiştirilemedi.";
      setError(errorMsg);
      // Hata durumunda değişikliği geri al (opsiyonel)
      setCards((prevCards) =>
        prevCards.map((c) =>
          c.id === cardId ? { ...c, isActive: currentStatus } : c
        )
      );
    } finally {
  ...
  const handleOpenQrModal = (card) => {
    ...
    const publicUrl = `${window.location.origin}/card/${
      card.customSlug || card.id
    }`;
              ...
              <React.Fragment key={card.id}>
                <ListItem alignItems="flex-start">
                  <ListItemAvatar>
                    <Avatar
                      alt={card.cardName}
                      src={card.profileImageUrl || undefined}
                    >
                  ...
                  <Box sx={{ display: "flex", alignItems: "center", gap: 0.5 }}>
                    <Tooltip
                      title={card.isActive ? "Kart Aktif" : "Kart Pasif"}
                    >
                      <FormControlLabel
                        control={
                          <Switch
                            ...
                            onChange={() =>
                              handleToggleStatus(card.id, card.isActive)
                            }
                    ...
                    <Tooltip title="QR Kod">
                      <IconButton
                        edge="end"
                        aria-label="qr-code"
                        onClick={() => handleOpenQrModal(card)}
